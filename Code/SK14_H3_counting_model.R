...
full_experiment_dataset <- read.csv("enteries_countingmodel.csv")

# Extract raw count data for single experimental unit
single_experimental_unit_label <- full_experiment_dataset[1,1]
single_experimental_unit <- as.numeric(full_experiment_dataset[1,2:133]) []#pull out a single row, and this is the place to iterate
...
MLE_Uniform_CxCoM <- matrix(nrow=0, ncol=39)
colnames(MLE_Uniform_CxCoM)<-c("ID",
                               "MLE_X_length",
                               "MLE_x1",
                               "MLE_x2",
                               "MLE_x3",
...
for (v in 1:max_p_value) {
  ...
  MLE_outputs <- c(single_experimental_unit_label,
                   (((Uniform_CxCoM_Table$par[1]+Uniform_CxCoM_Table$par[2]+Uniform_CxCoM_Table$par[3]+Uniform_CxCoM_Table$par[4]+Uniform_CxCoM_Table$par[5])/(2*p[1]))*100),
...
write.csv(df2,"/Users/spencerkoury/Desktop/local_protocols/missing_data_analyses/SK14_H3_full_output.csv",row.names = TRUE)
